...
const Select = createReactClass({
  ...
  propTypes: {
    ...
    // for screenreaders to press backspace to remove the current item - {label} is replaced with the item label
  ...
  handleInputBlur (event) {
    // The check for menu.contains(activeElement) is necessary to
  ...
  getValueArray (value, nextProps) {
    /** support optionally passing in the `nextProps` so
     * `componentWillReceiveProps` updates will function as expected */
  ...
  */
  ...
  setValue (value) {
    ...
    if (this.props.simpleValue && value) {
      value = this.props.multi ? value.map(i => i[this.props.valueKey]).join(this.props.delimiter)
  ...
  renderValue (valueArray, isOpen) {
    ...
    if (this.props.multi && !this.state.inputValue) {
      if (isOpen) onClick = null;
      const valueLabel = valueArray.length > 1
        ? valueArray.length + ' items selected'
      ...
      //
      // if (valueArray instanceof Array && valueArray.length == 1) {
      //   let value = valueArray[0];
      //   return <div>{renderLabel(value, 0)}</div>;
      // } else {
      //   return <div>{ valueArray.length + ' items selected' }</div>;
      // }
  ...
  renderInput (valueArray, focusedOptionIndex) {
    if (this.props.inputRenderer) {
      ...
      const ariaOwns = classNames({
        ...
        [this._instancePrefix + '-backspace-remove-message']: false &&
     !this.props.disabled &&
     ...
     !this.state.inputValue
      ...
      const inputProps = Object.assign({}, this.props.inputProps, {
        ...
        'aria-activedescendant': isOpen ? this._instancePrefix + '-option-' + focusedOptionIndex
  ...
  renderClear () {
    if (!this.props.clearable || (!this.props.value || this.props.value === 0) ||
      (this.props.multi && !this.props.value.length) || this.props.disabled || this.props.isLoading) return;
    return (
  ...
  render () {
    const valueArray = this.getValueArray(this.props.value);
    const options = this._visibleOptions = this.filterOptions(null);
