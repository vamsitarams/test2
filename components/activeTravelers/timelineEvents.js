...
// import flightsStatus from '../../redux/modules/flightsStatus';
// import CarTimelineEvents from './cartimeEvent';

export class TimelineEvents extends React.Component {
  static contextTypes = {
  ...
  setEvents () {
    const { productsData, carData, hotelsData } = this.props.productsData;
    const timelineEvents = carData.timelineEvents.concat(productsData.timelineEvents.concat(hotelsData.timelineEvents));
    const timelineEvent = timelineEvents.sort((a, b) => a.eventDateTime - b.eventDateTime);
    const travelerTimelineEvents = timelineEvent || [];
  ...
  get timeline () {
    ...
    console.log(this.state.currentUpcoming, 'timelinecurtent');
    console.log(this.state.pastEvent, 'timelinepastevent');
    const eventsArr = [].concat(this.state.pastEvent.concat(this.state.currentUpcoming));
    ...
    eventsArr.forEach((event, i) => {
      ...
      if (type === 'flight') {
        ...
        console.log(flightStatusObj, 'mms');
        if (flightStatusObj.travelerStatus === 'warning' || flightStatusObj.travelerStatus === 'alarm' || flightStatusObj.travelerStatus === 'ok')
        {
          let dirrection = null;
        ...
        <li key={iterator}>
          ...
          <div className='event-info'>
            ...
            <div className='text-holder'>
              ...
              {/* <p>{event.productRaw.flightState}</p> */}
            </div>
          </div>
        </li>
      );
